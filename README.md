...
- Swift 工具链（Xcode 15+ 或对应版本的 Swift toolchain）
...
首次启动时，macOS 会弹窗请求辅助功能权限。请前往 **系统设置 → 隐私与安全性 → 辅助功能**，勾选 cqlb，然后重新启动应用。
...
2. **输入切换** — Option+Space 切换中/英文模式；也可使用 Shift 快速单击（<300ms）切换
3. **编码匹配** — 击键送入引擎，查询超强两笔码表，匹配候选词
4. **文本注入** — 选中候选词后，通过 `CGEventPost` 将文本注入当前应用
5. **候选窗口** — 使用 AppKit 绘制候选窗口，跟随当前光标位置显示

切换快捷键：
- **Option+Space** — 主切换键，开启/关闭中文输入
- **Shift 单击** — 快速单击左 Shift 或右 Shift（<300ms，中间不按其他键）也可切换
...
- **无内嵌预编辑（inline preedit）** — 编码显示在独立的候选窗口中，而非应用的文本输入框内。这是外挂输入法方案的固有限制。
